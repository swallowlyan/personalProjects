      ...
      <div id="app1">
            ...
            <div class="item-list-wrap">
              <div class="cart-item">
                ...
                <ul class="cart-item-list">
                  <li v-for="(goods,index) in productList" :key="index">
                    <div class="cart-tab-1">
                      <div class="cart-item-check">
                        <a href="javascript:void 0" class="item-check-btn" v-bind:class="{'check':goods.checked}" @click="changeCheck(goods)">
                          <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                        </a>
                      </div>
                      <div class="cart-item-pic">
                        <img :src="goods.productImage" alt="烟">
                      </div>
                      ...
                      <div class="item-include">
                        <dl>
                          <dt>赠送:</dt>
                          <dd v-for="(part,index) in goods.parts" :key="index">{{part.partsName}}</dd>
                        </dl>
                      </div>
                    </div>
                    <div class="cart-tab-2">
                      <div class="item-price">{{goods.productPrice | formartMoney}}</div>
                    </div>
                    <div class="cart-tab-3">
                      <div class="item-quantity">
                        <div class="select-self select-self-open">
                          <div class="quantity">
                            <a href="javascript:void 0" @click="changeMoney(goods,-1)">-</a>
                            <input type="text" value="0"  v-model="goods.productQuantity">
                            <a href="javascript:void 0" @click="changeMoney(goods,1)">+</a>
                          </div>
                        </div>
                    ...
                    <div class="cart-tab-4">
                      <div class="item-price-total">{{goods.productPrice * goods.productQuantity | formartMoney}}</div>
                    </div>
                    <div class="cart-tab-5">
                      <div class="cart-item-operation">
                        <a href="javascript:void 0" class="item-edit-btn" @click="delGoods(goods)">
            ...
            <div class="cart-foot-wrap">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascript:void 0">
                    <span class="item-check-btn" :class="{'check':checkAll}" @click="checkedAll(true)">
                      <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                    </span>
                    <span>全选</span>
                  </a>
                ...
                <div class="item-all-del">
                  <a href="javascript:void 0" class="item-del-btn" @click="checkedAll(false)">
                    <span>取消全选</span>
                  </a>
                </div>
              </div>
              <div class="cart-foot-r">
                <div class="item-total">
                  Item total: <span class="total-price">{{totalPrice | formartMoney}}</span>
                </div>
                <div class="next-btn-wrap">
                  <span  class="btn btn--red" style="width: 200px" @click="goAddress()">结账</span>
                </div>
              </div>
            </div>  
          </div>
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModal" :class="{'md-show':delFlag}">
          <div class="md-modal-inner">
            <div class="md-top">
              <button class="md-close" @click="closeWin()">关闭</button>
            </div>
            <div class="md-content">
              ...
              <div class="btn-wrap col-2">
                <button class="btn btn--m" id="btnModalConfirm" @click="confirmDel()">Yes</button>
                <button class="btn btn--m btn--red" id="btnModalCancel" @click="closeWin()">No</button>
              </div>
            </div>
          </div>
        </div>
        <div  :class="{'md-overlay':overlay}" @click="closeWin()"></div>
      </div>
    </div>
</template>
...
// import Vue from 'vue'   全局注册set需要导入vue
export default {
  data() {
    return {
      totalMoney: 0,
  ...
  methods: {
    goodsShow() {
      ...
      this.$axios({
        ...
        console.log(res);
        // let {totalMoney} = res.data.result
        this.totalMoney = res.data.result.totalMoney;
        this.productList = res.data.result.list;
      });
    },
    changeMoney(val, flag) {
      if (flag > 0) {
        val.productQuantity++;
      } else {
        val.productQuantity--;
    ...
    changeCheck(goods) {
      if (typeof goods.checked == "undefined") {
        //  Vue.set(goods,'checked',true);   全局注册
        //局部注册
